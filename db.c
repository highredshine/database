...
// edit: pthread_rwlock object initialized.
...
node_t *node_constructor(char *arg_name, char *arg_value, node_t *arg_left,
    ...
    // edit: pthread_rwlock_init for the new field.
    if (pthread_rwlock_init(&new_node->lock, 0)) {
    	perror("pthread_rwlock_init for node_constructor failed. \n");
        return 0;
    }
...
void node_destructor(node_t *node) {
    // edit: pthread_rwlock_destroy for the new field.
    if (pthread_rwlock_destroy(&node->lock)) {
        perror("pthread_rwlock_destroy for pthread_rwlock_destroy failed. \n");
        exit(1);
    }
...
void db_query(char *name, char *result, int len) {
    ...
    if (target == 0) {
        ...
        // edit: unlock the target node if search was successful.
        if (pthread_rwlock_unlock(&target->lock) == EPERM) {
            perror("pthread_rwlock_unlock on target node for db_query failed. \n");
            exit(1);
        }
        return;
    }
...
int db_add(char *name, char *value) {
    ...
    if ((target = search(name, &head, &parent, 1)) != 0) {
        // edit: unlock the parent and target nodes if search failed.
        if (pthread_rwlock_unlock(&parent->lock) == EPERM){
        	perror("pthread_rwlock_unlock on parent node for db_add failed. \n");
            exit(1);
        }
        if (pthread_rwlock_unlock(&target->lock) == EPERM){
        	perror("pthread_rwlock_unlock on target node for db_add failed. \n");
            exit(1);
        }
        return (0);
    }
    ...
    // edit: unlock the parent of the newly added node if add was successful.
    if (pthread_rwlock_unlock(&parent->lock) == EPERM){
        perror("pthread_rwlock_unlock on parent node for db_add failed. \n");
        exit(1);
    }
...
int db_remove(char *name) {
    ...
    if ((dnode = search(name, &head, &parent, 1)) == 0) {
        // edit: unlock the parent if search failed.
        if (pthread_rwlock_unlock(&parent->lock) == EPERM){
        	perror("pthread_rwlock_unlock on parent node for db_remove failed. \n");
            exit(1);
        }
        // it's not there
        return (0);
    ...
    if (dnode->rchild == 0) {
        ...
        // edit: unlock parent node and dnode
        if (pthread_rwlock_unlock(&parent->lock) == EPERM){
        	perror("pthread_rwlock_unlock on parent node for db_remove failed. \n");
            exit(1);
        }
        if (pthread_rwlock_unlock(&dnode->lock) == EPERM){
        	perror("pthread_rwlock_unlock on dnode for db_remove failed. \n");
            exit(1);
        }
        // done with dnode
        ...
        next = dnode->rchild;
        // edit: wrlock the next node. 
        if (pthread_rwlock_wrlock(&next->lock) == EDEADLK) {
            perror("pthread_rwlock_wrlock on next node for db_remove failed. \n");
            exit(1);
        }
        ...
        while (next->lchild != 0) {
            ...
            // edit: unlock the next node and wrlock its left child.
            if (pthread_rwlock_unlock(&next->lock) == EPERM) {
                perror("pthread_rwlock_unlock on next node for db_remove failed. \n");
                exit(1);
            }
            if (pthread_rwlock_wrlock(&nextl->lock) == EDEADLK) {
                perror("pthread_rwlock_wrlock on nextl node for db_remove failed. \n");
                exit(1);
            }
            pnext = &next->lchild;
            next = nextl;
        ...
        *pnext = next->rchild;
        ...
        // edit: unlock parent, dnode, and next.
        if (pthread_rwlock_unlock(&parent->lock) == EPERM){
        	perror("pthread_rwlock_unlock on parent node for db_remove failed. \n");
            exit(1);
        }
...
node_t *search(char *name, node_t *parent, node_t **parentpp, int lock_type) {
    ...
    // edit: lock root node of the binary tree.
    if (strcmp(parent->name,"") == 0) {
        if (lock_type == 0) {
            if (pthread_rwlock_rdlock(&head.lock) == EDEADLK) {
                perror("pthread_rwlock_rdlock on root node for search failed. \n");
                exit(1);
            }
        } else if (lock_type == 1) {
            if (pthread_rwlock_wrlock(&head.lock) == EDEADLK) {
                perror("pthread_rwlock_wrlock on root node for search failed. \n");
                exit(1);
            }
        }
    }
    ...
    if (next == NULL) {
        ...
        // edit: lock the next node according to the lock type.
        if (lock_type == 0) {
            if (pthread_rwlock_rdlock(&next->lock) == EDEADLK) {
                perror("pthread_rwlock_rdlock on next node for search failed. \n");
                exit(1);
            }
        } else if (lock_type == 1) {
            if (pthread_rwlock_wrlock(&next->lock) == EDEADLK) {
                perror("pthread_rwlock_wrlock on next node for search failed. \n");
                exit(1);
            }
        }
        if (strcmp(name, next->name) == 0) {
            ...
            // edit: unlock parent node is search on this node failed.
            if (pthread_rwlock_unlock(&parent->lock) == EPERM) {
                perror("pthread_rwlock_unlock on parent node for search failed. \n");
                exit(1);
            }  
    ...
    if (parentpp != NULL) {
        *parentpp = parent;
        ...
        // edit: unlock parent if parent pointer is null.
        if (pthread_rwlock_unlock(&parent->lock) == EPERM) {
            perror("pthread_rwlock_unlock on parent node for search failed. \n");
            exit(1);
        }  
